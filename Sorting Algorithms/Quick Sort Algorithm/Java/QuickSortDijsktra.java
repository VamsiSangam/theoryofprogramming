...
Quick sort algorithm with Dijkstra's 3-way paritioning
...
public class QuickSortDijsktra{
    ...
    public static void quickSortDijsktra(int[] array, int low, int high){
    	...
    	// The loop here divides the Array in three different part
    	for (int i = low + 1; i <= high; i++) {

    		// This is to make sure that we don't get array out of bounds exceptions
    		if (lt == ht) {
    			break;
    		}

    		// Swap the left hand element with the i-th element if the i-th element
    		// is lesser than the pivot
    		// else swap the right hand element with the i-th element if the i-th element
